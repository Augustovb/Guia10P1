		...
		*pestado=0;
		...
		switch(c)
		{
			...
			case 'A':
					...
					matresults=solve2x2(matcoef);								//FALTA VER LO QUE HIZO gonzalocastelli  PARA LAS FUNCIONES SOLVE

					printf("Sus valores de las variables x,y respectivamente son :\n");
					printmatrix(matresults);
					break;
			...
			case 'b':
					...
					printf("Sus valores de las variables x y z respectivamente son:\n");
					printmatrix(matresults);
...
float * getmat3x3 (void)
{
	...
	coeficientes=makematrix(filas,columnas);
...
float * getmat2x2(void)				//generara una matriz de 2x3 con como si estuviesen las ecuaciones
{
	...
	int filas=2;
	int columnas=3;		//creo variables tipo int con la cantidad de filas y columnas, para luego pedir el espacio
